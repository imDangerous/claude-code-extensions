...
- **자동 테스트 + 문서 정리 (2026-06-19 21차)**: B) ccx 첫 자동 테스트 — node:test 블랙박스 10개(version/list/web init/variant/createOnly/apply 2계층·멱등/모듈 주소지정/backend init/git-branching opt-in/blockId 마이그레이션) 전부 통과. npm test=build+node --test. CI 워크플로(ci.yml) 추가(push/PR에 node --test). C) README 최신화(backend 실재·3타겟·git-hooks 위치·@import 2계층), orchestrate concern을 실제 자산에 맞게 수정(없는 backend 에이전트 미참조), architecture.html 갱신(backend→core·js 모듈·apply 2계층 예시). 미해결: entropy 측정 자동화·backend 빌드도구(Gradle/Maven)·observability 도구 고정도·backend 에이전트 네이밍.
- **프로덕션 버그픽스 (2026-06-19 22차, 실프로젝트 적용 피드백)**: v1 rules/git 등 레거시 훅 블록이 마이그레이션 시 제거 안 돼 orphan(삭제 파일 호출→커밋 실패), doctor도 정상으로 오판. → 훅 타깃에 soleOwner 도입: 현재 id 외 모든 ccx 블록을 stale로 제거(어떤 레거시 id든 self-heal), classify가 foreign 블록 감지→UPDATE라 doctor가 검출. js/git-hooks 두 훅에 soleOwner. 테스트 2개 추가(rules/git orphan 제거·doctor 검출), 12/12 통과. **거버넌스 메모**: 폐기된 core.md(승인 게이트)·git-branching.md(readypost feat/RP-·develop/main·RC·hotfix)는 조직 정책이라 ccx 밖이 맞음 — 초기 커밋 a9d04d9에 보존, 필요 시 ccx 관리 밖 이름으로 복구.
