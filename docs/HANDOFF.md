...
- **프로덕션 버그픽스 (2026-06-19 22차, 실프로젝트 적용 피드백)**: v1  등 레거시 훅 블록이 마이그레이션 시 제거 안 돼 orphan(삭제 파일 호출→커밋 실패), doctor도 정상으로 오판. → 훅 타깃에  도입: 현재 id 외 모든 ccx 블록을 stale로 제거(어떤 레거시 id든 self-heal), classify가 foreign 블록 감지→UPDATE라 doctor가 검출. js/git-hooks 두 훅에 soleOwner. 테스트 2개 추가(rules/git orphan 제거·doctor 검출), 12/12 통과. **거버넌스 메모**: 폐기된 core.md(승인 게이트)·git-branching.md(readypost feat/RP-·develop/main·RC·hotfix)는 조직 정책이라 ccx 밖이 맞음 — 초기 커밋 a9d04d9에 보존, 필요 시 ccx 관리 밖 이름으로 복구.
